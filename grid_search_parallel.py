...
def run_single_experiment(params: Dict, data_dir: str) -> Dict[str, float]:
    """
    단일 실험 실행 (병렬화를 위한 독립 함수)
    ...
    try:
        ...
        y_pred = np.concatenate([
            test_normal_preds,
            np.array(all_attack_preds)  # ← 수정: 리스트를 numpy array로 변환
        ])
        ...
        logger.error(f"Experiment failed with params {params}: {str(e)}")
        return {'error': str(e)}
...
class GridSearchHMM:
    ...
    def run_grid_search(self, param_grid: Dict[str, List]) -> pd.DataFrame:
        ...
        """
        logger.info("=" * 80)
        logger.info("Starting Grid Search (PARALLEL)")
        logger.info("=" * 80)
        logger.info(f"\nParameter Grid:")
        ...
        results_list = Parallel(n_jobs=self.n_jobs, verbose=10)(
            delayed(run_single_experiment)(params, self.data_dir)
            for params in param_list
        )

        # 결과 저장
        for params, metrics in zip(param_list, results_list):
    ...
    def plot_results(self, results_df: pd.DataFrame, save_path: str = None):
        ...
        fig.suptitle('Grid Search Results (Parallel)', fontsize=16)
...
def main():
    """메인 함수"""
    logger.info("=" * 80)
    logger.info("HMM Parameter Grid Search (PARALLEL VERSION)")
    logger.info("=" * 80)
    ...
    # 그리드 서치 실행 (n_jobs=-1: 모든 코어 사용)
    searcher = GridSearchHMM(data_dir='adfa-ld/ADFA-LD', n_jobs=-1)
